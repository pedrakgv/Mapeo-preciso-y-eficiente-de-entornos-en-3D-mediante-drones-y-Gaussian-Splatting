...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    // Obtencion de frame de grabacion
     const unsigned char *image_data = wb_camera_get_image(camera);
     int width = wb_camera_get_width(camera);
     int height = wb_camera_get_height(camera);

     if ((int)time % 10 == 0) {  // Guardar cada 10 segundos
      std::string filename = "image_" + std::to_string((int)time) + ".png";
      
      wb_camera_save_image(camera, filename.c_str(), 100);  //100 es el nivel de calidad
